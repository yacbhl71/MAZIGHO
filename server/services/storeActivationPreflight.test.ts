...
describe("store activation preflight", () => {
  it("is locally ready only when every local prerequisite is met", () => {
    ...
    expect(result.checks.filter(check => check.state === "manual").map(check => check.key)).toEqual(["domain_manual", "operator_confirmation"]);
  });
  ...
  it("blocks a setup boutique until owner, own branding, legal profile and catalogue are complete", () => {
    ...
    expect(result.checks.filter(check => check.state === "blocked").map(check => check.key)).toEqual(expect.arrayContaining(["owner", "brand", "legal", "catalogue"]));
  });
