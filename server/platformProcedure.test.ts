...
describe("MAZIGHO Studio platform guard", () => {
  it("refuses platform controls and the Studio inventory from a client storefront administrator", async () => {
    ...
    await expect(caller.admin.studio.saveOwnerExistingCatalogueProduct({ storeId: 1, productId: 1, categoryId: 1, name: "Produit privé", description: "Description privée.", longDescription: "Description détaillée privée.", priceCents: 1990, stock: 3, featured: false })).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(caller.admin.studio.createOwnerExistingCatalogueProduct({ storeId: 1, categoryId: 1, name: "Produit privé", description: "Description privée.", longDescription: "Description détaillée privée.", priceCents: 1990, stock: 3, featured: false })).rejects.toMatchObject({ code: "FORBIDDEN" });
