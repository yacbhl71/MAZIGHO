...
export default function AdminStudioOwnerPublicStorefrontContent({ storeIdOverride, params: routeParams }: { storeIdOverride?: number; params?: { storeId?: string } } = {}) {
  ...
  const applyVioletCraftTemplate = trpc.admin.studio.applyVioletCraftStorefrontTemplate.useMutation({ onSuccess: async ({ profile: saved }) => { setProfile(saved as DesignProfile); setNotice("Modèle Atelier créatif violet appliqué. Chaque texte, image et onglet reste modifiable ensuite."); await refresh(); }, onError: error => setNotice(error.message || "Le modèle n’a pas pu être appliqué.") });
  ...
  const isSaving = saveProfile.isPending || applyVioletCraftTemplate.isPending || saveBanner.isPending || deleteBanner.isPending || uploadImage.isPending;
  ...
  return <DashboardLayout><main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 md:px-8 md:py-8">
    ...
    {!isValidStoreId ? <Card className="border-rose-200 bg-rose-50"><CardContent className="p-5 text-sm text-rose-950">Boutique non sélectionnée.</CardContent></Card> : contentQuery.isLoading || !profile ? <div className="grid gap-5"><div className="h-52 animate-pulse rounded-2xl bg-slate-100" /><div className="h-80 animate-pulse rounded-2xl bg-slate-100" /></div> : contentQuery.isError ? <Card className="border-rose-200 bg-rose-50"><CardContent className="p-5 text-sm text-rose-950">Contenu indisponible : {contentQuery.error.message}</CardContent></Card> : <>
      ...
      <Card className="border-violet-200 bg-violet-50/70"><CardContent className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-violet-700">Modèle de départ optionnel</p><p className="mt-2 text-lg font-semibold text-violet-950">Atelier créatif violet</p><p className="mt-1 max-w-3xl text-sm leading-6 text-violet-900">Applique une identité Dyama, cinq visuels optimisés, un carrousel Diamond Painting, des textes loisirs créatifs et un menu thématique. Il remplace uniquement la vitrine de cette boutique ; chaque réglage reste modifiable ou masquable ensuite.</p></div><Button type="button" disabled={isSaving} onClick={() => applyVioletCraftTemplate.mutate({ storeId })} className="min-h-11 shrink-0 bg-violet-700 hover:bg-violet-800"><WandSparkles className="mr-2 h-4 w-4" />{applyVioletCraftTemplate.isPending ? "Application…" : "Appliquer ce modèle"}</Button></CardContent></Card>
