...
import { ArrowLeft, Check, CircleAlert, Eye, FileText, ImagePlus, Loader2, LockKeyhole, PanelTop, Save, Sparkles, X } from "lucide-react";
...
type DraftPage = { id: PageId; label: string; description: string; enabled: boolean; coverImageUrl: string; blocks: DraftBlock[] };
...
export default function AdminStudioOwnerPageEditor() {
  ...
  const uploadMutation = trpc.admin.studio.uploadOwnerPageImage.useMutation({
    onSuccess: result => {
      updatePage(page => ({ ...page, coverImageUrl: result.url }));
      setNotice("Image prête. Enregistrez ensuite le brouillon de page pour la conserver dans cet aperçu privé.");
    },
    onError: error => setNotice(error.message || "Impossible de préparer cette image."),
  });
  ...
  const handleImageSelected = (file: File | undefined) => {
    ...
    if (!["image/jpeg", "image/png", "image/webp"].includes(file.type)) {
      setNotice("Utilisez une image JPEG, PNG ou WebP.");
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setNotice("L’image doit peser au maximum 5 Mo.");
      return;
    }
    ...
    reader.onload = () => {
      ...
      setNotice("");
      uploadMutation.mutate({ storeId, dataUrl: reader.result, fileName: file.name });
    };
    reader.onerror = () => setNotice("Impossible de lire cette image.");
    reader.readAsDataURL(file);
  };

  const canSave = Boolean(selectedPage) && selectedPage.blocks.every(block => block.title.trim() && block.body.trim()) && !saveMutation.isPending && !uploadMutation.isPending;
      ...
      <main className="mx-auto w-full max-w-[1540px] space-y-6 px-4 py-6 md:px-8 md:py-8">
        ...
        {!isValidStoreId ? <Card className="border-rose-200 bg-rose-50"><CardContent className="flex gap-3 p-5 text-sm leading-6 text-rose-950"><CircleAlert className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-semibold">Boutique non sélectionnée.</p><p className="mt-1">Revenez dans MAZIGHO Studio et ouvrez l’éditeur depuis une boutique offerte en préparation.</p></div></CardContent></Card> : draftsQuery.isLoading ? <EditorSkeleton /> : draftsQuery.isError || !draftsQuery.data || !selectedPage ? <Card className="border-rose-200 bg-rose-50"><CardContent className="flex gap-3 p-5 text-sm leading-6 text-rose-950"><CircleAlert className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-semibold">Éditeur de pages indisponible.</p><p className="mt-1">Cette page est réservée à une boutique offerte en état `setup`, depuis MAZIGHO Studio uniquement.</p></div></CardContent></Card> : <>
          ...
          <section className="grid gap-5 xl:grid-cols-[240px_minmax(0,1fr)_350px]">
            ...
            <div className="space-y-5"><Card className="border-slate-200"><CardHeader><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><CardDescription>Page sélectionnée</CardDescription><CardTitle className="mt-1 text-2xl">{selectedPage.label}</CardTitle><p className="mt-2 text-sm leading-6 text-slate-600">{selectedPage.description}</p></div><label className="flex w-fit cursor-pointer items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700"><input type="checkbox" checked={selectedPage.enabled} onChange={event => updatePage(page => ({ ...page, enabled: event.target.checked }))} className="h-4 w-4 rounded border-slate-300 text-violet-700 focus:ring-violet-600" /> Préparer cette page</label></div></CardHeader><CardContent className="space-y-5"><section className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="flex items-center gap-2 text-sm font-semibold text-slate-950"><ImagePlus className="h-4 w-4 text-violet-700" /> Image de couverture</p><p className="mt-1 max-w-xl text-sm leading-6 text-slate-600">Ajoutez une image JPEG, PNG ou WebP de 5 Mo maximum. Elle est utilisée seulement dans le brouillon et l’aperçu privé de cette page.</p></div><label className={`inline-flex w-fit cursor-pointer items-center rounded-lg px-3 py-2 text-sm font-semibold ${uploadMutation.isPending ? "bg-slate-200 text-slate-500" : "bg-slate-950 text-white hover:bg-slate-800"}`}><input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={uploadMutation.isPending} onChange={event => { handleImageSelected(event.target.files?.[0]); event.currentTarget.value = ""; }} />{uploadMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ImagePlus className="mr-2 h-4 w-4" />}{uploadMutation.isPending ? "Préparation…" : "Choisir une image"}</label></div>{selectedPage.coverImageUrl && <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-white"><img src={selectedPage.coverImageUrl} alt="Aperçu de l’image de couverture" className="h-52 w-full object-cover" /><div className="flex items-center justify-between gap-3 p-3"><p className="text-xs text-slate-500">Image préparée dans le brouillon</p><Button type="button" size="sm" variant="outline" className="border-slate-300 bg-white" onClick={() => updatePage(page => ({ ...page, coverImageUrl: "" }))}><X className="mr-1.5 h-3.5 w-3.5" /> Retirer</Button></div></div>}</section>{selectedPage.blocks.map((block, index) => <section key={block.id} className={`rounded-2xl border p-4 ${block.visible ? "border-slate-200 bg-white" : "border-slate-200 bg-slate-50 opacity-70"}`}><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2"><span className="grid h-6 w-6 place-items-center rounded-full bg-slate-950 text-xs font-bold text-white">{index + 1}</span><p className="text-sm font-semibold text-slate-950">{block.label}</p></div><label className="flex cursor-pointer items-center gap-2 text-xs font-semibold text-slate-600"><input type="checkbox" checked={block.visible} onChange={event => updateBlock(block.id, current => ({ ...current, visible: event.target.checked }))} className="h-4 w-4 rounded border-slate-300 text-violet-700 focus:ring-violet-600" /> Visible dans l’aperçu</label></div><div className="mt-4 space-y-2"><Label htmlFor={`${block.id}-title`}>Titre du bloc</Label><Input id={`${block.id}-title`} value={block.title} maxLength={120} onChange={event => updateBlock(block.id, current => ({ ...current, title: event.target.value }))} /></div><div className="mt-4 space-y-2"><Label htmlFor={`${block.id}-body`}>Texte</Label><Textarea id={`${block.id}-body`} value={block.body} maxLength={1200} onChange={event => updateBlock(block.id, current => ({ ...current, body: event.target.value }))} className="min-h-[108px] resize-y" /></div></section>)}</CardContent></Card>
