...
export default function AdminOrders() {
  ...
  const decide = trpc.admin.orders.decide.useMutation({
    onSuccess: async (_, variables) => {
      ...
      setIsDetailsOpen(false);
      await Promise.all([refetch(), refetchDecisions(), refetchFulfillment()]);
    },
    onError: error => toast.error(error.message || "Décision impossible."),
  });

  const reconcileStripeTestAndAccept = trpc.admin.orders.reconcileStripeTestAndAccept.useMutation({
    onSuccess: async () => {
      toast.success("Paiement Stripe Test vérifié, commande acceptée et synchronisations relancées.");
      setDecisionTarget(null);
      setDecisionReason("");
      setDecisionConfirmation("");
      setIsDetailsOpen(false);
    ...
    onError: error => toast.error(error.message || "La vérification Stripe Test a échoué."),
  });
  ...
  const submitDecision = () => {
    ...
    const isUnreconciledStripeTest = decisionTarget.action === "accepted"
      && selectedOrder?.paymentMethod === "stripe_test"
      && selectedOrder?.paymentStatus !== "paid";
    if (isUnreconciledStripeTest) {
      reconcileStripeTestAndAccept.mutate({ orderId: decisionTarget.orderId });
      return;
    }
    decide.mutate({
  ...
  const downloadAliExpressManifest = () => {
    if (!aliExpressManifest?.shipping || !aliExpressManifest.lines.length) return;
    const payload = {
      schemaVersion: 1,
      source: "MAZIGHO",
      purpose: "Préparation manuelle AliExpress avant panier et paiement humain",
      orderId: aliExpressManifest.orderId,
      paymentConfirmedInMazigho: aliExpressManifest.paymentConfirmed,
      paymentPolicy: aliExpressManifest.paymentPolicy,
      shipping: {
        name: aliExpressManifest.shipping.name,
        line1: aliExpressManifest.shipping.line1,
        line2: aliExpressManifest.shipping.line2,
        city: aliExpressManifest.shipping.city,
        state: aliExpressManifest.shipping.state,
        postalCode: aliExpressManifest.shipping.postalCode,
        countryCode: aliExpressManifest.shipping.countryCode,
        phone: aliExpressManifest.shipping.phone,
      },
      lines: aliExpressManifest.lines.map(line => ({
        ...
        intendedCountryCode: line.intendedCountryCode,
        optionStatus: line.optionStatus,
        urlSource: line.urlSource,
      })),
      safeguards: [
        "Ce fichier est local : il n’envoie aucune donnée à AliExpress.",
        "Vérifier manuellement la fiche, la variante, le prix, la disponibilité, la livraison et l’adresse avant toute action.",
        "Le panier et le paiement AliExpress exigent une validation humaine distincte.",
      ],
    };
    ...
    link.href = url;
    link.download = `mazigho-aliexpress-preparation-${aliExpressManifest.orderId}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    toast.success("Manifeste AliExpress téléchargé localement. Aucun envoi ni paiement n’a été lancé.");
  };
      ...
      <div className="space-y-6 pb-8">
        <section className="rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 via-white to-cyan-50 p-6 md:p-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex items-center gap-2 rounded-xl border border-blue-100 bg-white px-4 py-3 text-sm text-slate-700"><CircleDollarSign className="h-5 w-5 text-blue-600" /> Les montants de vente affichent la devise réellement facturée ; les coûts fournisseur restent internes.</div>
          </div>
        </section>
              ...
              <TableBody>
                {isLoading ? Array.from({ length: 5 }).map((_, index) => <TableRow key={index}>{Array.from({ length: 8 }).map((__, cell) => <TableCell key={cell}><Skeleton className="h-5 w-full" /></TableCell>)}</TableRow>) : filteredOrders.length === 0 ? (
                  ...
                  <TableRow key={order.id} className="hover:bg-slate-50/70">
                    <TableCell><div className="flex items-center gap-3"><div className="rounded-xl bg-blue-50 p-2 text-blue-700"><ShoppingBag className="h-4 w-4" /></div><div><p className="font-semibold text-slate-900">#{order.id}</p><p className="text-xs text-muted-foreground">{formatPrice(order.totalAmount, "fr", order.currencyCode || "CHF")}</p></div></div></TableCell>
            ...
            {selectedOrder && <form onSubmit={handleUpdateStatus}>
              ...
              <div className="grid gap-5 py-5">
                ...
                <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2 font-semibold text-slate-900"><ShieldAlert className="h-4 w-4 text-blue-700" /> Validation avant fournisseur</div><p className="mt-1 max-w-xl text-xs leading-5 text-slate-600">Aucune décision ci-dessous ne crée une commande CJ, ne transmet l’adresse client ou n’effectue un remboursement réel. Ces actions restent manuelles pendant la phase de test.</p></div><Badge variant="outline" className="border-blue-200 bg-white text-blue-800">Sas actif</Badge></div>
                  <div className="mt-4 grid gap-2 sm:grid-cols-3"><Button type="button" variant="outline" className="border-emerald-200 bg-white text-emerald-800 hover:bg-emerald-50" disabled={selectedOrder.status !== "pending" || (selectedOrder.paymentStatus !== "paid" && selectedOrder.paymentMethod !== "stripe_test")} onClick={() => openDecision("accepted")}><CheckCircle className="mr-2 h-4 w-4" /> {selectedOrder.paymentStatus !== "paid" && selectedOrder.paymentMethod === "stripe_test" ? "Vérifier & accepter (test)" : "Accepter"}</Button><Button type="button" variant="outline" className="border-rose-200 bg-white text-rose-800 hover:bg-rose-50" disabled={selectedOrder.status === "shipped" || selectedOrder.status === "delivered"} onClick={() => openDecision("rejected")}><XCircle className="mr-2 h-4 w-4" /> Refuser</Button><Button type="button" variant="outline" className="border-amber-200 bg-white text-amber-900 hover:bg-amber-50" disabled={selectedOrder.paymentStatus !== "paid" || selectedOrder.paymentStatus === "refunded"} onClick={() => openDecision("refund_requested")}><RotateCcw className="mr-2 h-4 w-4" /> Demander remboursement</Button></div>
                  {selectedOrder.paymentStatus !== "paid" && selectedOrder.paymentMethod === "stripe_test" && <p className="mt-3 text-xs text-blue-700">Pour Stripe Test, MAZIGHO vérifiera d’abord la session directement auprès de Stripe. L’acceptation reste refusée si Stripe ne la confirme pas comme payée.</p>}
                  {selectedOrder.paymentStatus !== "paid" && selectedOrder.paymentMethod !== "stripe_test" && <p className="mt-3 text-xs text-slate-500">L’acceptation est disponible uniquement après la confirmation d’un paiement réel.</p>}
