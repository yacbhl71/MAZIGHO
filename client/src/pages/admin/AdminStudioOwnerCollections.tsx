...
function ActiveStoreManagementFromCollections({ storeId, storeName, domain }: { storeId: number; storeName: string; domain: string | null }) {
  ...
  const issueTemporaryPassword = trpc.admin.studio.issueOwnerTemporaryPassword.useMutation({
    onSuccess: result => {
  ...
  return <DashboardLayout><main className="mx-auto w-full max-w-6xl space-y-6 px-4 py-6 md:px-8 md:py-8">
    ...
    <Card className="border-amber-200 bg-amber-50"><CardHeader><CardDescription>Secours d’accès propriétaire</CardDescription><CardTitle className="mt-1">Générer un mot de passe temporaire</CardTitle></CardHeader><CardContent className="space-y-4"><p className="max-w-3xl text-sm leading-6 text-amber-950">Utilisez cette action uniquement si le propriétaire ne peut pas définir son mot de passe avec son lien d’invitation ou de réinitialisation. Le mot de passe généré est affiché une fois, n’est jamais envoyé par e-mail et doit être changé après connexion.</p>{temporaryPassword ? <div className="space-y-3 rounded-xl border border-amber-300 bg-white p-4"><Label htmlFor="temporary-password" className="text-sm font-bold text-slate-950">Mot de passe temporaire — copiez-le maintenant</Label><Input id="temporary-password" value={temporaryPassword} readOnly className="mt-2 border-amber-300 bg-amber-50 font-mono text-sm text-slate-950" /><p className="text-xs leading-5 text-slate-600">Après connexion sur le panneau de cette boutique, le propriétaire doit le remplacer immédiatement dans les réglages du compte.</p></div> : <div className="flex flex-col gap-3 sm:flex-row sm:items-end"><div className="w-full max-w-md"><Label htmlFor="owner-access-email" className="text-sm font-bold text-slate-950">Adresse du propriétaire à confirmer</Label><Input id="owner-access-email" type="email" value={ownerEmail} onChange={event => setOwnerEmail(event.target.value)} placeholder="proprietaire@exemple.ch" className="mt-2 border-amber-300 bg-white" /></div><Button type="button" disabled={!ownerEmail.includes("@") || issueTemporaryPassword.isPending} onClick={() => issueTemporaryPassword.mutate({ storeId, confirmationEmail: ownerEmail, acknowledged: true })} className="min-h-11 bg-amber-700 text-white hover:bg-amber-800">{issueTemporaryPassword.isPending ? "Génération…" : "Générer l’accès temporaire"}</Button></div>}</CardContent></Card>
  </main></DashboardLayout>;
}
