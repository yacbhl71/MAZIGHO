...
export default function AdminStudioOwnerCataloguePublication() {
  ...
  const canConfirmActivation = Boolean(activation?.locallyReadyForManualActivation && activationName.trim() === activationQuery.data?.store.displayName && activationOwnerEmail.trim() && domainVerified && activationAcknowledged && !activateMutation.isPending);
...
function ActivationPanel({ activation, storeName, name, setName, ownerEmail, setOwnerEmail, domainVerified, setDomainVerified, acknowledged, setAcknowledged, enabled, pending, error, success, onActivate }: { activation: { checks: ActivationCheck[]; locallyReadyForManualActivation: boolean } | undefined; storeName: string; name: string; setName: (value: string) => void; ownerEmail: string; setOwnerEmail: (value: string) => void; domainVerified: boolean; setDomainVerified: (value: boolean) => void; acknowledged: boolean; setAcknowledged: (value: boolean) => void; enabled: boolean; pending: boolean; error?: string; success: boolean; onActivate: () => void }) {
