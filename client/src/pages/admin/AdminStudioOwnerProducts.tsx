...
import { ArrowLeft, CircleAlert, Eye, LockKeyhole, PackagePlus, Plus, Save, Sparkles, Trash2, Truck, WalletCards } from "lucide-react";
    ...
    {query.isLoading ? <div className="grid gap-5 xl:grid-cols-[minmax(0,1.12fr)_390px]"><div className="h-[620px] animate-pulse rounded-3xl bg-slate-100" /><div className="h-[420px] animate-pulse rounded-3xl bg-slate-100" /></div> : query.isError || !query.data ? <Unavailable /> : <>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.12fr)_390px]">
        <Card className="border-slate-200 shadow-sm"><CardHeader className="border-b border-slate-100"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><CardDescription>Offre de préparation</CardDescription><CardTitle className="mt-1 text-xl">Vos futures fiches</CardTitle></div><Badge variant="outline" className="w-fit border-slate-300 bg-slate-50 text-slate-700">{products.length} / 24</Badge></div></CardHeader><CardContent className="space-y-4 p-4 sm:p-6">
          {!collections.length ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><p className="font-bold">Préparez d’abord une collection.</p><p className="mt-1">Une fiche produit doit être rattachée à une collection de présentation. Revenez à l’atelier précédent, créez au moins une collection, puis enregistrez-la.</p></div> : products.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center"><WalletCards className="mx-auto h-7 w-7 text-slate-500" /><p className="mt-3 font-bold text-slate-950">Aucune fiche produit préparée</p><p className="mx-auto mt-1 max-w-md text-sm leading-6 text-slate-600">Commencez par une fiche claire. Vous pourrez ensuite compléter la préparation de stock et de référence fournisseur dans l’atelier dédié, toujours sans action externe.</p></div> : null}
