...
export default function AdminSuppliers() {
  ...
  const runCustomSourcing = async () => {
    ...
    if (!window.confirm(`Lancer un sourcing CJ pour « ${customKeyword.trim()} » ? MAZIGHO cherchera au plus ${requestedProducts} brouillons classés dans ${categoryNames.join(", ")}, contrôlés pour ${countryNames.join(", ")}, sous ${maxWeightG} g, avec un prix final ×${priceMultiplier} et uniquement des méthodes CJPacket rapides. Chaque brouillon conservera uniquement les pays où sa livraison est réellement validée ; il restera masqué dans les autres pays. Aucun produit ne sera publié ou commandé.`)) return;
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-700">Définissez une niche, une catégorie, le pays de vente, le volume souhaité et vos règles commerciales. MAZIGHO ne retient que les produits dont le stock, le poids, le fret et le prix final sont validés pour la destination choisie.</p>
              </div>
            </div>
