...
export default function AdminSuppliers() {
  ...
  const cjGlobalWarehousesQuery = trpc.admin.suppliers.cjGlobalWarehouses.useQuery(undefined, { enabled: Boolean(cjStatus?.configured), retry: false });
  ...
  const runCustomSourcing = async () => {
    ...
    const maxWeightG = Number.parseInt(customMaxWeightG, 10);
    const priceMultiplier = Number.parseFloat(customPriceMultiplier.replace(",", "."));
    const limits = customSourcingConfigQuery.data?.limits;
    if (!customKeyword.trim() || customKeyword.trim().length < 2) {
    ...
    if (customShippingMethodIds.length === 0) {
      toast.error("Cochez au moins une famille de transport.");
      return;
    }
    ...
    if (!Number.isInteger(draftLimit) || draftLimit < limits.minDraftLimit || draftLimit > limits.maxDraftLimit) {
      toast.error(`La limite de brouillons doit être comprise entre ${limits.minDraftLimit} et ${limits.maxDraftLimit}.`);
      return;
    }
    if (!Number.isInteger(maxWeightG) || maxWeightG < limits.minWeightG || maxWeightG > limits.maxWeightG) {
      toast.error(`Le poids maximal doit être compris entre ${limits.minWeightG} g et ${limits.maxWeightG.toLocaleString("fr-CH")} g.`);
      return;
    }
    if (!Number.isFinite(priceMultiplier) || priceMultiplier < limits.minPriceMultiplier || priceMultiplier > limits.maxPriceMultiplier) {
    ...
    const shippingLabels = (customSourcingConfigQuery.data?.shippingMethods || []).filter(method => customShippingMethodIds.includes(method.id)).map(method => method.label);
    if (!window.confirm(`Lancer un sourcing CJ pour « ${customKeyword.trim()} » ? MAZIGHO cherchera au plus ${requestedProducts} brouillons classés dans ${categoryNames.join(", ")}, contrôlés pour ${countryNames.join(", ")}, depuis ${warehouseLabels.join(", ")}, avec ${shippingLabels.join(" · ")}, sous ${maxWeightG} g et un prix final ×${priceMultiplier}. Chaque brouillon conservera uniquement les pays où sa livraison est réellement validée ; il restera masqué dans les autres pays. Aucun produit ne sera publié ou commandé.`)) return;
    await importCjCustomDrafts.mutateAsync({
      ...
      warehouseCountryCodes: customWarehouseCountryCodes,
      shippingMethodIds: customShippingMethodIds,
      requestedProducts,
      draftLimit,
      maxWeightG,
      priceMultiplier,
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-6xl space-y-6 pb-8">
        ...
        <section className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 shadow-sm" data-testid="cj-legacy-tools-card">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2 text-sm font-semibold text-slate-800"><LockKeyhole className="h-4 w-4 text-slate-600" /> Outils classiques / sauvegarde</div><p className="mt-1 text-xs leading-5 text-slate-600">Les anciens lots fixes High-Tech, Mode et catégories restent conservés, mais sont repliés afin de laisser le générateur personnalisé au premier plan.</p></div><Button type="button" variant="outline" onClick={() => setLegacyToolsOpen(current => !current)} className="border-slate-300 bg-white text-slate-800 hover:bg-slate-100">{legacyToolsOpen ? "Masquer les outils classiques" : "Afficher les outils classiques"}</Button></div>
        </section>

        {legacyToolsOpen && <section className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-5 shadow-sm md:p-6" data-testid="cj-batch-import-card">
        ...
        <section className="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-sky-50 p-5 shadow-sm md:p-6" data-testid="cj-custom-sourcing-card">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex gap-4">
              <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-violet-100 text-violet-700"><SlidersHorizontal className="h-5 w-5" /></div>
              <div>
                <div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold text-slate-950">Générateur de Sourcing CJ sur-mesure</h2><Badge className="bg-violet-100 text-violet-800 hover:bg-violet-100">Brouillons uniquement</Badge></div>
                <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-700">Définissez une niche, les catégories, les pays de vente, l’entrepôt d’origine et les transports admis. MAZIGHO ne retient que les produits dont le stock, le poids, le fret et le prix final sont validés pays par pays.</p>
              </div>
            </div>
            <Badge variant="outline" className="w-fit border-violet-200 bg-white text-violet-800">Contrôle humain requis</Badge>
          </div>

          <form onSubmit={event => { event.preventDefault(); void runCustomSourcing(); }} className="mt-5 grid gap-4">
            <datalist id="cj-custom-niches">
              <option value="Gadgets" />
              <option value="Accessoires téléphone" />
              <option value="Organisation maison" />
              <option value="Fitness léger" />
              <option value="Mode" />
            ...
            <div className="grid gap-4 xl:grid-cols-2">
              <fieldset className="grid gap-1.5"><legend className="flex items-center gap-1.5 text-sm font-semibold text-slate-800"><Warehouse className="h-4 w-4 text-violet-700" /> Entrepôt CJ d’origine <span className="font-normal text-slate-500">(facultatif)</span></legend><div className="grid max-h-36 grid-cols-2 gap-1.5 overflow-y-auto rounded-xl border border-violet-200 bg-white p-2"><label className="col-span-2 flex cursor-pointer items-center gap-2 rounded-lg bg-violet-50 px-2 py-1.5 text-sm font-medium text-violet-950"><input type="checkbox" checked={customWarehouseCountryCodes.length === 0} onChange={event => { if (event.target.checked) setCustomWarehouseCountryCodes([]); }} className="h-4 w-4 rounded border-violet-300 text-violet-700 focus:ring-violet-500" /><span>Tous les entrepôts CJ compatibles</span></label>{cjGlobalWarehousesQuery.isLoading ? <p className="col-span-2 px-2 py-2 text-xs text-slate-500">Lecture des entrepôts CJ…</p> : cjGlobalWarehousesQuery.data?.map(warehouse => <label key={warehouse.countryCode} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-sm text-slate-800 hover:bg-violet-50"><input type="checkbox" checked={customWarehouseCountryCodes.includes(warehouse.countryCode)} onChange={event => setCustomWarehouseCountryCodes(current => event.target.checked ? [...current, warehouse.countryCode] : current.filter(code => code !== warehouse.countryCode))} className="h-4 w-4 rounded border-violet-300 text-violet-700 focus:ring-violet-500" /><span className="min-w-0 truncate">{warehouse.label}</span><span className="ml-auto text-xs text-slate-400">{warehouse.countryCode}</span></label>)}</div><span className="text-xs text-slate-500">Une préférence retient seulement les variantes dont CJ confirme l’origine sélectionnée. Elle ne change jamais les pays de vente cochés plus haut.</span>{cjGlobalWarehousesQuery.isError && <span className="text-xs text-amber-700">Liste CJ indisponible : le sourcing reste possible avec tous les entrepôts compatibles.</span>}</fieldset>
              <fieldset className="grid gap-1.5"><legend className="flex items-center gap-1.5 text-sm font-semibold text-slate-800"><Truck className="h-4 w-4 text-violet-700" /> Modes de livraison admis</legend><div className="grid gap-1.5 rounded-xl border border-violet-200 bg-white p-2">{(customSourcingConfigQuery.data?.shippingMethods || []).map(method => <label key={method.id} className="flex cursor-pointer items-start gap-2 rounded-lg px-2 py-1.5 text-sm text-slate-800 hover:bg-violet-50"><input type="checkbox" checked={customShippingMethodIds.includes(method.id)} onChange={event => setCustomShippingMethodIds(current => event.target.checked ? [...current, method.id] : current.filter(id => id !== method.id))} className="mt-0.5 h-4 w-4 rounded border-violet-300 text-violet-700 focus:ring-violet-500" /><span><span className="block font-medium">{method.label}</span><span className="block text-xs text-slate-500">{method.description}</span></span></label>)}</div><span className="text-xs text-slate-500">Le devis réel CJ doit confirmer le canal et le délai. Les noms postaux, économiques, ordinaires ou explicitement non suivis restent rejetés.</span></fieldset>
            </div>

            <div className="flex flex-col gap-3 rounded-xl border border-violet-200 bg-white/80 p-4 md:flex-row md:items-center md:justify-between"><p className="max-w-3xl text-xs leading-5 text-slate-700"><strong>Règles non négociables :</strong> produit CJ unique, catégorie standard, stock positif dans l’entrepôt éventuellement choisi, poids connu sous le seuil, fret chiffré par pays, méthode sélectionnée avec délai vérifié, prix client final tout compris et statut brouillon. Aucun article ne sera publié, payé ou commandé chez CJ.</p><Button type="submit" disabled={!cjStatus?.configured || customSourcingConfigQuery.isLoading || importCjCustomDrafts.isPending} className="shrink-0 bg-violet-700 text-white hover:bg-violet-800">{importCjCustomDrafts.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <PackageSearch className="mr-2 h-4 w-4" />}{importCjCustomDrafts.isPending ? "Sourcing en cours…" : "Lancer le sourcing personnalisé"}</Button></div>
          </form>

          {customSourcingResult && <div className="mt-4 grid gap-3 lg:grid-cols-[1.2fr_0.8fr]"><div className="rounded-xl border border-violet-200 bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-semibold text-slate-950">Dernier sourcing : « {customSourcingResult.keyword} »</p><p className="mt-1 text-sm text-slate-600">{customSourcingResult.categoryNames.join(" · ")} · {customSourcingResult.countryNames.join(" · ")} · {customSourcingResult.warehouseCountryCodes.length ? customSourcingResult.warehouseCountryCodes.join(" · ") : "tous entrepôts compatibles"} · {customSourcingResult.shippingMethodLabels.join(" · ")} · {customSourcingResult.maxWeightG} g max. · prix ×{customSourcingResult.priceMultiplier}</p></div><Badge className="bg-violet-100 text-violet-800 hover:bg-violet-100">{customSourcingResult.imported}/{customSourcingResult.requested} brouillon(s)</Badge></div>{customSourcingResult.products.length > 0 ? <div className="mt-3 divide-y rounded-lg border border-violet-100">{customSourcingResult.products.map(product => <div key={product.id} className="flex flex-wrap items-center justify-between gap-3 px-3 py-2 text-sm"><div className="min-w-0"><span className="block truncate font-medium text-slate-900">{product.name}</span><span className="text-xs text-slate-500">Visible : {product.countryCodes.map(code => customSourcingConfigQuery.data?.countries.find(country => country.countryCode === code)?.countryName || code).join(" · ")}</span></div><span className="shrink-0 text-violet-800">{(product.priceCents / 100).toFixed(2)} CHF</span></div>)}</div> : <p className="mt-3 text-sm text-slate-600">Aucun candidat n’a passé tous les contrôles. Aucun brouillon n’a été créé.</p>}</div><div className="rounded-xl border border-violet-200 bg-violet-50/70 p-4 text-sm text-violet-950"><p className="font-semibold">Synthèse du contrôle</p><p className="mt-2">{customSourcingResult.existingCounts.map(item => `${item.categoryName} : ${item.count}`).join(" · ")} · plafond {customSourcingResult.draftLimit} par catégorie</p><p className="mt-1">{customSourcingResult.skipped} candidat(s) écarté(s) · {customSourcingResult.failures.length} incident(s) CJ.</p><p className="mt-3 text-xs leading-5 text-violet-900">Les incidents et candidats écartés ne créent ni produit public ni commande fournisseur.</p></div></div>}
        </section>
