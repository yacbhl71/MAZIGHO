...
const DEFAULT_CUSTOM_SOURCING_RULES: CustomSourcingRules = {
  ...
  enforceSelectedShippingMethods: true,
  enforceMaxDeliveryDays: true,
  maxDeliveryDays: 15,
};
...
export default function AdminSuppliers() {
  ...
  const runCustomSourcing = async () => {
    ...
    if (customRules.enforceSelectedShippingMethods && customShippingMethodIds.length === 0) {
      toast.error("Cochez au moins une famille de transport ou désactivez cette règle.");
      return;
    }
    ...
    if (customRules.enforceMaxWeight && (!Number.isInteger(maxWeightG) || maxWeightG < limits.minWeightG || maxWeightG > limits.maxWeightG)) {
      toast.error(`Le poids maximal doit être compris entre ${limits.minWeightG} g et ${limits.maxWeightG.toLocaleString("fr-CH")} g.`);
      return;
    }
    if (customRules.enforceMaxDeliveryDays && (!Number.isInteger(customRules.maxDeliveryDays) || customRules.maxDeliveryDays < 1 || customRules.maxDeliveryDays > limits.maxDeliveryDays)) {
      toast.error(`Le délai maximal doit être compris entre 1 et ${limits.maxDeliveryDays} jours.`);
      return;
    }
        ...
        <section className="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-sky-50 p-5 shadow-sm md:p-6" data-testid="cj-custom-sourcing-card">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-700">Définissez la niche et vos destinations, puis activez uniquement les filtres utiles dans la colonne de règles. Les critères sélectionnés sont affichés et enregistrés avec chaque sourcing.</p>
              </div>
            </div>
          ...
          <form onSubmit={event => { event.preventDefault(); void runCustomSourcing(); }} className="mt-5 grid gap-4">
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              ...
              <label className={`grid gap-1.5 ${customRules.enforceMaxWeight ? "" : "opacity-55"}`}><span className="flex items-center gap-1.5 text-sm font-semibold text-slate-800"><Weight className="h-4 w-4 text-violet-700" /> Poids maximum (g)</span><input type="number" inputMode="numeric" disabled={!customRules.enforceMaxWeight} min={customSourcingConfigQuery.data?.limits.minWeightG || 50} max={customSourcingConfigQuery.data?.limits.maxWeightG || 10000} value={customMaxWeightG} onChange={event => setCustomMaxWeightG(event.target.value)} className="h-11 rounded-xl border border-violet-200 bg-white px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-violet-500 disabled:cursor-not-allowed disabled:bg-slate-100" /><span className="text-xs text-slate-500">{customRules.enforceMaxWeight ? "Les variantes au-dessus du seuil ou sans poids sont rejetées." : "Filtre désactivé : le poids n’écarte aucun candidat."}</span></label>
