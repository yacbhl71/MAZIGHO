...
export default function AdminCustomization() {
  ...
  const initialTab = ["style", "menu", "home", "images", "catalogue", "product", "checkout", "pages", "sections", "footer"].includes(hashTab) ? hashTab : "style";
    ...
    <DashboardLayout>
      <div className="space-y-6 pb-8">
        ...
        <Tabs key={initialTab} defaultValue={initialTab} className="space-y-5">
          <TabsList className="h-auto flex-wrap justify-start gap-1 bg-transparent p-0"><TabsTrigger value="style" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><Palette className="h-4 w-4" /> Style</TabsTrigger><TabsTrigger value="menu" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><LayoutPanelTop className="h-4 w-4" /> Menu</TabsTrigger><TabsTrigger value="home" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><LayoutPanelTop className="h-4 w-4" /> Accueil</TabsTrigger><TabsTrigger value="images" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><ImagePlus className="h-4 w-4" /> Images</TabsTrigger><TabsTrigger value="catalogue" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><Sparkles className="h-4 w-4" /> Pages catalogue</TabsTrigger><TabsTrigger value="product" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><ShieldCheck className="h-4 w-4" /> Fiche produit</TabsTrigger><TabsTrigger value="checkout" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><ShoppingCart className="h-4 w-4" /> Panier & commande</TabsTrigger><TabsTrigger value="pages" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><FileText className="h-4 w-4" /> Pages publiques</TabsTrigger><TabsTrigger value="sections" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><Eye className="h-4 w-4" /> Sections</TabsTrigger><TabsTrigger value="footer" className="gap-2 border data-[state=active]:border-violet-200 data-[state=active]:bg-violet-50"><LayoutPanelTop className="h-4 w-4" /> Footer</TabsTrigger></TabsList>
          ...
          <TabsContent value="checkout" className="space-y-5">
            <Card className="shadow-sm">
              <CardHeader><CardTitle className="flex items-center gap-2"><ShoppingCart className="h-5 w-5 text-violet-600" /> Panier et commande</CardTitle><CardDescription>Modifiez les titres et explications vus par vos visiteurs avant une demande de paiement. Ces champs ne configurent aucun paiement, encaissement, e-mail ou commande réelle.</CardDescription></CardHeader>
              <CardContent className="grid gap-5 lg:grid-cols-2">
                <section className="rounded-xl border border-sky-100 bg-sky-50/50 p-4"><p className="font-semibold text-slate-950">Panier</p><p className="mt-1 text-xs leading-5 text-slate-600">Utilisez <code>{"{country}"}</code> pour afficher le pays de livraison sélectionné.</p><div className="mt-4 space-y-4"><div className="space-y-2"><Label>Petit libellé</Label><Input value={form.cartEyebrow} maxLength={120} onChange={event => setField("cartEyebrow", event.target.value)} /></div><div className="space-y-2"><Label>Titre</Label><Input value={form.cartTitle} maxLength={120} onChange={event => setField("cartTitle", event.target.value)} /></div><div className="space-y-2"><Label>Introduction</Label><textarea value={form.cartIntro} maxLength={360} onChange={event => setField("cartIntro", event.target.value)} className="min-h-24 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" /></div></div></section>
                <section className="rounded-xl border border-violet-100 bg-violet-50/50 p-4"><p className="font-semibold text-slate-950">Commande</p><p className="mt-1 text-xs leading-5 text-slate-600">Gardez un message transparent tant que les paiements ne sont pas activés.</p><div className="mt-4 space-y-4"><div className="space-y-2"><Label>Petit libellé</Label><Input value={form.checkoutEyebrow} maxLength={120} onChange={event => setField("checkoutEyebrow", event.target.value)} /></div><div className="space-y-2"><Label>Titre</Label><Input value={form.checkoutTitle} maxLength={120} onChange={event => setField("checkoutTitle", event.target.value)} /></div><div className="space-y-2"><Label>Introduction</Label><textarea value={form.checkoutIntro} maxLength={420} onChange={event => setField("checkoutIntro", event.target.value)} className="min-h-20 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" /></div><div className="space-y-2"><Label>Information de paiement</Label><textarea value={form.checkoutPaymentNotice} maxLength={420} onChange={event => setField("checkoutPaymentNotice", event.target.value)} className="min-h-20 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" /></div></div></section>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="pages" className="space-y-5">
            <Card className="shadow-sm">
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-violet-600" /> Pages publiques</CardTitle>
                <CardDescription>
                  Configurez la FAQ, Contact, Livraison & retours et À propos de la boutique principale. Ces pages sont aussi autonomes que celles des boutiques clientes et restent protégées dans MAZIGHO Studio.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <StoreSystemPagesEditor scope="platform" />
              </CardContent>
            </Card>
          </TabsContent>
