...
export default function AdminCjImport() {
  ...
  const suggestedCostCents = useMemo(() => {
    ...
    if (!preparedProduct || preparedProduct.supplierPriceUsd == null || !Number.isFinite(rate) || rate <= 0) return null;
    return Math.round(preparedProduct.supplierPriceUsd * rate * 100);
  }, [exchangeRate, preparedProduct]);
        ...
        {preparedProduct && <form onSubmit={handleSave} className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_330px]">
          <section className="space-y-5 rounded-xl border bg-white p-5 shadow-sm">
            ...
            <div className="space-y-4 rounded-xl border border-sky-100 bg-sky-50/60 p-4">
              <div><h3 className="font-semibold text-slate-900">Prix et conversion à valider</h3><p className="mt-1 text-xs leading-5 text-slate-600">Le taux n’est pas rempli automatiquement : renseignez une valeur et une source que vous avez vérifiée. Le coût peut ensuite être ajusté manuellement.</p></div>
              <div className="grid gap-4 md:grid-cols-2">
