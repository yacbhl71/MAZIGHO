...
import StudioStoreCommercialSupervision from "@/components/StudioStoreCommercialSupervision";
import StudioDomainConnectionGuide from "@/components/StudioDomainConnectionGuide";
...
export default function AdminStudioOwnerActiveStoreManagement() {
  ...
  const supervisionQuery = trpc.admin.studio.getStoreCommercialSupervision.useQuery({ storeId: validStoreId ? storeId : 0 }, { enabled: validStoreId, retry: false, refetchOnWindowFocus: false });
  ...
  const store = storefront?.store ?? catalogue?.store;
  ...
  return <DashboardLayout><main className="mx-auto w-full max-w-6xl space-y-6 px-4 py-6 md:px-8 md:py-8">
    ...
    {loading ? <div className="grid gap-4 md:grid-cols-2"><div className="h-52 animate-pulse rounded-2xl bg-slate-100" /><div className="h-52 animate-pulse rounded-2xl bg-slate-100" /></div> : unavailable ? <Card className="border-rose-200 bg-rose-50"><CardContent className="flex gap-3 p-5 text-sm leading-6 text-rose-950"><TriangleAlert className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-bold">Gestion indisponible.</p><p className="mt-1">La boutique n’a pas pu être identifiée dans Studio. Vérifiez le registre des boutiques.</p></div></CardContent></Card> : <>
      <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-white text-emerald-700"><Store className="h-5 w-5" /></div><div><p className="font-bold text-emerald-950">{store?.displayName ?? "Boutique"}</p><p className="mt-1 text-sm text-emerald-800">Statut : {store?.status ?? "à vérifier"} · Domaine : {domain || "à configurer"}</p></div></div>{domain && <Button asChild variant="outline" className="min-h-11 border-emerald-300 bg-white text-emerald-950 hover:bg-emerald-100"><a href={`https://${domain}`} target="_blank" rel="noreferrer"><ExternalLink className="mr-2 h-4 w-4" /> Voir la boutique</a></Button>}</div></section>
      <StudioStoreCommercialSupervision supervision={supervisionQuery.data} loading={supervisionQuery.isLoading || supervisionQuery.isFetching} error={supervisionQuery.isError} onRefresh={() => { void Promise.all([supervisionQuery.refetch(), storefrontQuery.refetch(), catalogueQuery.refetch()]); }} />
      <StudioDomainConnectionGuide storeId={storeId} storeName={store?.displayName ?? "la boutique"} currentDomain={supervisionQuery.data?.store.primaryDomain} recoveryDomain={supervisionQuery.data?.recoveryDomain} storeStatus={supervisionQuery.data?.store.status} preparationReady={preparationReady} request={supervisionQuery.data?.domainRequest} loading={supervisionQuery.isLoading || supervisionQuery.isFetching} onRefresh={() => { void Promise.all([supervisionQuery.refetch(), storefrontQuery.refetch(), catalogueQuery.refetch()]); }} />
      <section className="grid gap-5 md:grid-cols-2"><Card className="border-slate-200"><CardHeader><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-orange-50 text-orange-700"><PackageSearch className="h-5 w-5" /></div><div><CardDescription>Catalogue réel</CardDescription><CardTitle className="mt-1">Produits et catégories</CardTitle></div></div></CardHeader><CardContent><p className="text-sm leading-6 text-slate-600">{catalogue ? `${catalogue.categories.length} catégorie(s) et ${catalogue.products.length} produit(s) sont disponibles à l’édition.` : catalogueQuery.isError ? "Le résumé du catalogue est temporairement indisponible ; l’éditeur reste accessible." : "Le catalogue se chargera ici."}</p><Button asChild className="mt-5 min-h-11 bg-slate-950 text-white hover:bg-slate-800"><a href={`/admin/studio/catalogue-existant/${storeId}`}><Boxes className="mr-2 h-4 w-4" /> Gérer le catalogue</a></Button></CardContent></Card>
        <Card className="border-slate-200"><CardHeader><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-violet-50 text-violet-700"><Palette className="h-5 w-5" /></div><div><CardDescription>Vitrine publique</CardDescription><CardTitle className="mt-1">Bannières, images et textes</CardTitle></div></div></CardHeader><CardContent><p className="text-sm leading-6 text-slate-600">{storefrontQuery.data ? `${storefrontQuery.data.banners.length} bannière(s) propre(s) sont enregistrées. Personnalisez l’identité et les visuels animalier.` : storefrontQuery.isError ? "Le résumé de vitrine est temporairement indisponible ; l’éditeur reste accessible." : "La personnalisation se chargera ici."}</p><Button asChild className="mt-5 min-h-11 bg-violet-700 text-white hover:bg-violet-800"><a href={`/admin/studio/contenu-public/${storeId}`}><Image className="mr-2 h-4 w-4" /> Personnaliser la vitrine</a></Button></CardContent></Card></section>
