...
export default function Checkout() {
  ...
  const subtotalChf = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
  const shippingEstimate = calculateCheckoutShippingEstimate(subtotalChf, shippingPolicyQuery.data);
  const subtotal = cart.reduce((sum, item) => sum + convertFromChf(item.price) * item.quantity, 0);
