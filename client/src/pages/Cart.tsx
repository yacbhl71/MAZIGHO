...
export default function Cart() {
  ...
  const cartEyebrow = locale === "fr" ? profile.cartEyebrow : c.selection;
  const cartTitle = locale === "fr" ? profile.cartTitle : c.cart;
  const cartIntro = locale === "fr" ? profile.cartIntro.replace(/\{country\}/g, countryLabel) : `${c.destination}: ${countryLabel}.`;
  ...
  const items = cart.map(item => {
    ...
    const reason = !product ? commerceT(locale, "productNotFoundText") : hasVariants && !selectedVariant ? "Cette variante n’est plus disponible." : stock < item.quantity ? commerceT(locale, "outOfStock") : !countryServed ? `La boutique ne propose pas encore la livraison vers ${countryLabel}.` : !deliveryAvailable ? commerceT(locale, "deliveryUnconfirmed", { country: countryLabel }) : null;
  ...
  const isReadyForReview = cart.length > 0 && !productsQuery.isLoading && !shippingPolicyQuery.isLoading && unavailableItems.length === 0;

  if (cart.length === 0) return <div className="flex min-h-screen flex-col bg-[#fbf7f2]"><Header /><main className="container flex flex-1 items-center justify-center px-4 py-20 text-center"><div className="max-w-md"><div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-orange-100 text-orange-600"><ShoppingBag className="h-9 w-9" /></div><p className="mb-3 text-xs font-bold uppercase tracking-[0.28em] text-orange-600">{cartEyebrow}</p><h1 className="text-3xl font-semibold tracking-tight text-slate-950">{c.empty}</h1><p className="mt-4 text-slate-600">{c.emptyText}</p><Button onClick={() => setLocation("/boutique")} className="mt-8 gap-2 bg-orange-500 text-white hover:bg-orange-600"><ArrowLeft className="h-4 w-4" /> {c.continueShopping}</Button></div></main><Footer /></div>;

  return <div className="flex min-h-screen flex-col bg-[#fbf7f2]"><Header /><main className="container flex-1 px-4 py-10 md:py-14"><div className="mb-10 flex flex-col gap-4 border-b border-[#eadfd2] pb-8 md:flex-row md:items-end md:justify-between"><div><p className="mb-3 text-xs font-bold uppercase tracking-[0.28em] text-orange-600">{cartEyebrow}</p><h1 className="text-4xl font-semibold tracking-tight text-slate-950">{cartTitle}</h1><p className="mt-2 text-sm text-slate-600">{cartIntro}</p></div><Button variant="outline" onClick={() => setLocation("/boutique")} className="w-fit gap-2 border-[#d9cbbc] bg-white"><ArrowLeft className="h-4 w-4" /> {c.continueShopping}</Button></div>
