...
function toDraft(profile: DesignProfile): HomepageDraft {
  return {
    ...
    closingVisualFont: profile.closingVisualFont,
    closingVisualColor: profile.closingVisualColor,
  ...
  return <Card className="border-violet-200">
    ...
    <CardContent className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-700 bg-slate-950 p-4 text-white">
        <div className="mb-4"><p className="font-semibold">Bloc final</p><p className="mt-1 text-xs leading-5 text-slate-300">Une dernière invitation à explorer ou à contacter votre boutique. Si le grand texte est vide, aucun texte n’est affiché : vous gardez un visuel entièrement libre.</p></div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label className="text-slate-200">Grand texte visuel</Label><Input value={draft.closingVisualValue} maxLength={40} onChange={event => setDraft(current => ({ ...current, closingVisualValue: event.target.value }))} placeholder="Facultatif — laissez vide pour ne rien afficher" className="border-slate-600 bg-white/10 text-white placeholder:text-slate-400" /><p className="text-xs leading-5 text-slate-400">Un champ vide reste vide sur la boutique : aucune devise n’est ajoutée automatiquement.</p></div><div className="space-y-2"><Label className="text-slate-200">Légende du visuel</Label><Input value={draft.closingVisualText} maxLength={280} onChange={event => setDraft(current => ({ ...current, closingVisualText: event.target.value }))} className="border-slate-600 bg-white/10 text-white" /></div></div>
        <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_minmax(0,.8fr)]"><div className="space-y-2"><Label className="text-slate-200">Police du grand texte</Label><select value={draft.closingVisualFont} onChange={event => setDraft(current => ({ ...current, closingVisualFont: event.target.value as HomepageDraft["closingVisualFont"] }))} className="min-h-11 w-full rounded-md border border-slate-600 bg-white/10 px-3 text-sm text-white"><option value="inherit" className="text-slate-950">Police de la boutique</option><option value="editorial" className="text-slate-950">Éditoriale élégante</option><option value="modern" className="text-slate-950">Moderne directe</option><option value="classic" className="text-slate-950">Classique raffinée</option></select></div><div className="space-y-2"><Label className="text-slate-200">Couleur du grand texte</Label><div className="flex min-h-11 items-center gap-3 rounded-md border border-slate-600 bg-white/10 px-3"><input type="color" aria-label="Choisir la couleur du grand texte" value={draft.closingVisualColor} onChange={event => setDraft(current => ({ ...current, closingVisualColor: event.target.value }))} className="h-8 w-12 cursor-pointer rounded border-0 bg-transparent p-0" /><span className="font-mono text-sm text-white">{draft.closingVisualColor.toUpperCase()}</span></div></div></div>
