...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "MAZIGHO_START_FULFILLMENT") {
    ...
    if (payload && payload.state && payload.state !== "ready_for_human_review") {
      sendResponse({ ok: false, error: "MANIFEST_NOT_READY" });
      return true;
    }
    const firstItem = payload && payload.lines && payload.lines.find((l) => l.supplierUrl);
    if (!firstItem) {
      sendResponse({ ok: false, error: "NO_SUPPLIER_URL" });
      return true;
    }
    chrome.tabs.create({ url: firstItem.supplierUrl, active: true }, (tab) => {
      if (tab && tab.id != null) {
