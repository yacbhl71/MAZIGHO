...
(function () {
  ...
  // ------------------------------- FLUX ---------------------------------
  async function run() {
    if (payload.state && payload.state !== "ready_for_human_review") {
      setStatus("⛔ Manifeste non prêt (" + (payload.reason || payload.state) + "). Corrigez la commande dans MAZIGHO.", "warn");
      report("blocked", { blocked: true, message: payload.reason || payload.state });
      return;
    }
    const lines = payload.lines || [];
    const item = lines.find((l) => l.supplierUrl) || lines[0];
    ...
    await fillShippingAddress(payload.shipping || {});
  ...
  async function selectVariant(item) {
    // Best-effort : on clique sur les valeurs d'options choisies par le client
    // (couleur/taille) issues du manifeste. Sinon on laisse l'utilisateur choisir.
    if (!item) return;
    const opts = item.selectedOptions && typeof item.selectedOptions === "object" ? Object.values(item.selectedOptions) : [];
    if (!opts.length) {
      setStatus("⚠️ Aucune option à sélectionner — vérifiez la variante manuellement si besoin.", "warn");
      return;
    }
    const wanted = opts.map((s) => String(s).trim().toLowerCase()).filter(Boolean);
  ...
  async function fillShippingAddress(addr) {
    ...
    await fill(S.checkout?.inputContactName, addr.name);
    ...
    await fill(S.checkout?.inputAddressLine1, addr.line1);
    await fill(S.checkout?.inputAddressLine2, addr.line2);
    await fill(S.checkout?.inputZipCode, addr.postalCode);
